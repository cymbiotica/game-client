...
var showModalMessage = function showModalMessage(type, error, player) {
...
var getGames = function getGames(over) {
  ...
  var url = over === false ? config.apiUrl + '/games?over=false' : config.apiUrl + '/games?over=true';
 ...
 * Return a promise.
 */
var updateGame = function updateGame(dataGame, index, value, isOver) {
...
var showNotCompleted = function showNotCompleted() {
  $('#list-games-not-complete').fadeIn();$('#game-board, #div-score').hide();
  gamelogic.resetGame();
  ui.showProgress();

  api.getGames(false).then(function (data) {
    ...
    $.each(data.games, function (index, element) {
      ...
      $('#btn-select-game' + index).click(function (e) {
        ...
        api.getGameById(element.id).then(function (result) {
...
var paintBoardGameCreated = function paintBoardGameCreated(size, game) {

  var parentElement = document.querySelector('.game-panel');
  var board = parentElement.querySelector('.board');

  grid_size = size;

  var arr = game.game.cells;
  ...
  var arrayList = chunkArray(arr, 3);
  var arrayOne = arrayList[0];
  var arrayTwo = arrayList[1];
  var arrayThree = arrayList[2];

  // bastard code begings here
  var table = '<table>';
  table += '<tr>';
  for (var _i = 0; _i < arrayOne.length; _i++) {

    table += '<td row="' + 0 + '" column="' + _i + '">' + arrayOne[_i].toUpperCase() + '</td>';
    data[0 + '' + _i] = arrayOne[_i];
  }
  ...
  for (var _i2 = 0; _i2 < arrayTwo.length; _i2++) {

    table += '<td row="' + 1 + '" column="' + _i2 + '">' + arrayTwo[_i2].toUpperCase() + '</td>';
    data[1 + '' + _i2] = arrayTwo[_i2];
  }
  ...
  for (var _i3 = 0; _i3 < arrayThree.length; _i3++) {

    table += '<td row="' + 2 + '" column="' + _i3 + '">' + arrayThree[_i3].toUpperCase() + '</td>';
    data[2 + '' + _i3] = arrayThree[_i3];
  }
  ...
  for (var _i4 = 0; _i4 < columns.length; _i4++) {
    columns[_i4].addEventListener('click', markElement);
  }
};
var paintBoard = function paintBoard(size) {
  ...
  api.creategame().then(function (data) {
    ...
    for (var _i5 = 0; _i5 < columns.length; _i5++) {
      columns[_i5].addEventListener('click', markElement);
    }
...
var markElement = function markElement() {
  ...
  var index = setCellValue(row, column, current_mark, gameData);
...
var updateCell = function updateCell(gameData, index, current_mark, isOver) {
  api.updateGame(gameData, index, current_mark, isOver).then(function (result) {
...
var checkForWin = function checkForWin(mark, grid_size) {
 ...
 */
var setCellValue = function setCellValue(row, column, current_mark, gamedata) {
...
module.exports = {
  ...
  resetGame: resetGame,
  paintBoardGameCreated: paintBoardGameCreated
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
